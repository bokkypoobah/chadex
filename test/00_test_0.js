...
describe("Dexz", function () {
  ...
  it("00. Test 00", async function () {
    ...
    const trade1adupTx = await data.dexz.connect(data.user0Signer).trade(Action.FillAnyAndAddOrder, BuySell.Sell, data.token0.address, data.weth.address, ethers.utils.parseUnits(price1, 9), expiry, baseTokens1, []);
    await data.printEvents("user0->dexz.trade(FillAnyAndAddOrder, SELL, token0, WETH, " + price1 + ", expiry, baseTokens1, [])", await trade1adupTx.wait());
