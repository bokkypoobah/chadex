...
describe("Dexz", function () {
  ...
  it("00. Test 00", async function () {
    ...
    const trade1sTx = await data.dexz.connect(data.user0Signer).trade(BUYSELL_BUY, FILL_ANY_AND_ADD_ORDER, data.token0.address, data.weth.address, price2, expiry, baseTokens, data.uiFeeAccount);
    await data.printEvents("user0->dexz.trade(BUY, FILL_ANY_AND_ADD_ORDER, token0, WETH, 0.6901, expiry, baseTokens, uiFeeAccount)", await trade1sTx.wait());
    ...
    const sellBaseTokens = ethers.utils.parseEther("2");

    const trade2sTx = await data.dexz.connect(data.user2Signer).trade(BUYSELL_SELL, FILL_ANY_AND_ADD_ORDER, data.token0.address, data.weth.address, price2, expiry, sellBaseTokens, data.uiFeeAccount);
    await data.printEvents("user2->dexz.trade(SELL, FILL_ANY_AND_ADD_ORDER, token0, WETH, 0.6901, expiry, sellBaseTokens, uiFeeAccount)", await trade2sTx.wait());
